...
is the ML knowledge graph stored in GraphDB, which is based on ML 
...
reviewed in Section~\ref{subsection:ml-ontology}, and because its 
...
Section~\ref{subsection:ml-ontology}, including the absence of 
...
They are described in more detail in this section.
...
of the extended ontology. The upper part shows the classes from the 
...
identified by its DOI and title. It is linked to 
...
\texttt{regression}, which belongs to \texttt{supervised\_learning} 
...
coverage gaps identified in Section~\ref{subsection:ml-ontology}. 
...
Section~\ref{subsection:ml-ontology} was applied to propagate 
...
extension and applying the inference rule. For reference, 
the original coverage across 122 instances is shown in 
Table~\ref{tab:ml-approach-coverage}. 
Coverage remains incomplete for several properties. Populating 
properties requires a lot of manual work for each 
instance, both for existing instances that lacked them and for 
all newly added ones. Effort on this was therefore prioritised towards 
reinforcement learning, which had no instances at all in the 
original ontology, and less towards filling gaps in the 
...
the user's problem description and the specific method.
