...
an HTTP API. The backend is implemented in FastAPI \parencite{fastapi}, 
...
\parencite[158]{sommerville2011software_engineering}. This keeps concerns 
...
approach also supports incremental development 
\parencite[158]{sommerville2011software_engineering}, which is consistent 
with the development process described in 
...
and user interaction. An integrations layer wraps calls to the backend 
API and external services, keeping these concerns separate from the 
application logic.
...
other concerns that run through most layers. User services covers 
authentication and saved searches, allowing users to store and reload 
previous searches with problem descriptions. Feedback handling allows users to rate recommended 
methods, with ratings stored in PostgreSQL and used to adjust method 
ranking over time. Both concerns follow the same layered structure as 
the recommendation logic, with separate modules in the router, service, 
and persistence layers of the backend, and corresponding wrappers in 
the frontend integrations layer.
