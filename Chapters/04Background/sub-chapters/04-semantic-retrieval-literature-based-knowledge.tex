...
\begin{itemize}
    \item Text represented as numerical vectors (embeddings) in a high-dimensional space
    \item Semantically similar texts receive similar vector representations and are close in embedding space
    \item Cosine similarity as the primary measure for comparing embeddings
\end{itemize}
...
space \parencite{jurafsky_martin2026speech_language_processing}. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Figures/embedding-space-example.png}
    \caption{A two-dimensional projection of example word embeddings. 
...
processing \parencite{jurafsky_martin2026speech_language_processing}. 
Rather than measuring absolute distance between two points, it 
measures the angle between two vectors, normalized for their length:
...
\parencite{jurafsky_martin2026speech_language_processing}. 
Normalizing for vector length ensures that the similarity score 
...
\begin{itemize}
    \item Transformer-based language models as the foundation for contextual text representations
    \item SBERT (Sentence-BERT): fine-tuned to produce fixed-size sentence embeddings suitable for cosine similarity comparison
    \item Sentence-level embeddings suited for comparing full problem descriptions against article abstracts
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Figures/sbert-architecture.png}
    \caption{SBERT architecture at inference time. Each sentence 
    is encoded independently through a shared BERT network, 
    producing vectors \textbf{u} and \textbf{v} that are compared 
    using cosine similarity. Adapted from 
...
\begin{itemize}
    \item Retrieval based on semantic meaning rather than exact keyword matching
    \item A query is embedded and compared against a corpus of pre-embedded documents
    \item More flexible than keyword search: captures related concepts even when exact terms differ
\end{itemize}
...
user query \parencite{jurafsky_martin2026speech_language_processing}. 
...
\parencite{jurafsky_martin2026speech_language_processing}. While 
...
\parencite{jurafsky_martin2026speech_language_processing}. Because 
...
Figure~\ref{fig:keyword-vs-dense} illustrates the distinction. 
The same query returns only one document under keyword-based 
retrieval, while dense retrieval ranks all three documents by 
semantic similarity, including those that share no exact terms 
with the query.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Figures/keyword-vs-dense.png}
    \caption{Keyword-based versus dense retrieval for the same query.}
    \label{fig:keyword-vs-dense}
\end{figure}

\subsection{Cross-Encoder Reranking}
\begin{itemize}
    \item Bi-encoders embed query and document independently, enabling efficient large-scale comparison
    \item Cross-encoders process query and document jointly, producing more precise relevance scores
    \item Two-stage pipeline: bi-encoder retrieval for speed, cross-encoder reranking for precision
    \item Trade-off: cross-encoders are more accurate but too slow for large-scale initial retrieval
\end{itemize}

\subsection{Scientific Literature as a Knowledge Source}
\begin{itemize}
    \item Scientific articles as a source of evidence for machine learning method recommendations
    \item Large volumes of unstructured literature make manual navigation impractical
    \item Semantic retrieval enables systematic matching between problem descriptions and relevant articles
\end{itemize}
